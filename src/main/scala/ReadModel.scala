...
trait ReadModelFacade {
  ...
  def findInventoryItemDetails(id: UUID): Option[InventoryItemDetailsDto]
}

class InMemReadModelFacade extends ReadModelFacade {
  def inventoryItems = BullShitDB.list
  def findInventoryItemDetails(id: UUID) = BullShitDB.details.get(id)
}
